  ...
  return (
    <>
      ...
      <GestureHandlerRootView style={{ marginTop: 50, paddingVertical: 10 }}>
        ...
        >
          <View style={{ flex: 1, flexDirection: 'row', padding: 10, alignItems: 'center', justifyContent: 'space-between', marginVertical: 10 }}>
            <Text style={style.heding}>Weights : {weights.length}</Text>
            {user && user?.assigned_permissions.includes('shoe_weight_create') && < Pressable
              style={style.button}
              onPress={
                () => {
                  setWeight(undefined)
              ...
              weights && weights.map((weight, index) => {
                return (
                  <View key={index} style={{ gap: 2, padding: 10, borderBottomWidth: 1, backgroundColor: 'whitesmoke', shadowRadius: 10, justifyContent: 'center' }}>
                    ...
                    {weight.shoe_weight1 && <Text style={style.label}>{`Weight1 : ${weight.shoe_weight1}-${weight.upper_weight1} = ${weight.shoe_weight1 - weight.upper_weight1}   | ${weight.weighttime1 && moment(new Date(weight.weighttime1)).format('LT')}`}</Text>}
                    {weight.shoe_weight2 && <Text style={style.label}>{`Weight2 : ${weight.shoe_weight2}-${weight.upper_weight2} = ${weight.shoe_weight2 - weight.upper_weight2}   | ${weight.weighttime2 && moment(new Date(weight.weighttime2)).format('LT')}`}</Text>}
                    {weight.shoe_weight3 && <Text style={style.label}>{`Weight3 : ${weight.shoe_weight3}-${weight.upper_weight3} = ${weight.shoe_weight3 - weight.upper_weight3}   | ${weight.weighttime3 && moment(new Date(weight.weighttime3)).format('LT')}`}</Text>}
                    ...
                    <View style={{ flex: 1, gap: 5, flexDirection: 'row' }}>
                      ...
                      < Pressable
                        style={!weight.shoe_weight1 ? style.circleredbutton : Math.abs(weight.shoe_weight1 - weight.upper_weight1 - weight.dye.stdshoe_weight) > 30 ? style.circleyellowbutton : style.circlebutton}
                        onPress={
                          () => {
                            setWeight(weight)
                            setChoice({ type: ProductionChoiceActions.create_showweight })
                        ...
                        disabled={isLoading}
                      >
                        <Text style={Math.abs(weight.shoe_weight1 - weight.upper_weight1 - weight.dye.stdshoe_weight) > 30 ? style.circlebuttonblacktext : style.circlebuttontext}>{Math.abs(weight.shoe_weight1 - weight.upper_weight1) - weight.dye.stdshoe_weight}</Text>
                      </Pressable>



                      < Pressable
                        style={!weight.shoe_weight2 ? style.circleredbutton : Math.abs(weight.shoe_weight2 - weight.upper_weight2 - weight.dye.stdshoe_weight) > 30 ? style.circleyellowbutton : style.circlebutton}
                        onPress={
                          () => {
                            setWeight(weight)
                        ...
                        disabled={isLoading}
                      >
                        <Text style={Math.abs(weight.shoe_weight2 - weight.upper_weight2 - weight.dye.stdshoe_weight) > 30 ? style.circlebuttonblacktext : style.circlebuttontext}>{Math.abs(weight.shoe_weight2 - weight.upper_weight2) - weight.dye.stdshoe_weight}</Text>
                      </Pressable>


                      < Pressable
                        style={!weight.shoe_weight3 ? style.circleredbutton : Math.abs(weight.shoe_weight3 - weight.upper_weight3 - weight.dye.stdshoe_weight) > 30 ? style.circleyellowbutton : style.circlebutton}
                        onPress={
                          () => {
                            setWeight(weight)
                        ...
                        disabled={isLoading}
                      >
                        <Text style={Math.abs(weight.shoe_weight3 - weight.upper_weight3 - weight.dye.stdshoe_weight) > 30 ? style.circlebuttonblacktext : style.circlebuttontext}>{Math.abs(weight.shoe_weight3 - weight.upper_weight3) - weight.dye.stdshoe_weight}</Text>
                      </Pressable>
                    </View>
                  </View>
                )
